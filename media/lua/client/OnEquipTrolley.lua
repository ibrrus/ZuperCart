    ...
    for playerNum = 0, playersSum - 1 do
        ...
        if playerObj then
                ...
                for i = 1, #TrolleyList do
                    ...
                    if itemsArray:size() > 0 then
                        ...
                        if trolForDrop then
                            ...
                            -- print("AddWorldInventoryItem 36")
            ...
            if playerObj:getVariableString("righthandmask") == "holdingtrolleyright" then
                ...
                if trol and trol:getContainer() == playerInv then
                    if not (playerObj:getCurrentState() == IdleState.instance() or 
                        ...
                        -- ISTimedActionQueue.add(ISUnequipAction:new(playerObj, playerObj:getPrimaryHandItem(), 1));
                        playerObj:setPrimaryHandItem(nil);
                        playerObj:setSecondaryHandItem(nil);
                        -- print("AddWorldInventoryItem 87")
                    ...
                    if playerObj:getVehicle() then
                        ...
                        if areaCenter then 
                            ...
                            playerObj:setPrimaryHandItem(nil);
                            playerObj:setSecondaryHandItem(nil);
                            -- print("AddWorldInventoryItem 106")
                            sqr:AddWorldInventoryItem(trol, 0, 0, 0);
                        end
                    end
                else
...
function isForceDropHeavyItem(item)
    -- print(item:getOutermostContainer())
    if item and (item:getFullType() == TrolleyList[1] or item:getFullType() == TrolleyList[2] or item:getFullType() == TrolleyList[3] or item:getFullType() == TrolleyList[4]) and item:getOutermostContainer() then
        return true
    else
